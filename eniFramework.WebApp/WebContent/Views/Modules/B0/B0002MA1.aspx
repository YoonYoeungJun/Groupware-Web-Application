<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/WebContent/Common/CommonPage.Master" CodeBehind="B0002MA1.aspx.cs" Inherits="eniFramework.WebApp.Modules.B0.B0002MA1" %>
...
<asp:Content ContentPlaceHolderID="Sub_Module_Description" runat="server">
    ...
    *  1. Module Name          : 시스템설정
    *  2. Function Name        :
    *  3. Program ID           : B0002MA1.aspx
    *  4. Program Name         : 시스템변경사항등록
    *  5. Program Desc         : 시스템변경사항등록
    *  6. Comproxy List        :
...
<asp:Content ContentPlaceHolderID="Sub_Script_Holder" runat="server">
    <script type="text/javascript">
        try {
            var B0002MA1 = eni.Initialize(new function () {
                lgPageRowCount = 100;
                ...
                var C_NO                = C_IDX++,
                    ...
                    C_SEQ               = C_IDX++,
                    C_PATCH_TYPE        = C_IDX++,
                    C_PATCH_TYPE_NM     = C_IDX++,
                    C_PATCH_CONTENT     = C_IDX++,
                    C_PATCH_REMARK      = C_IDX++,
                    C_PATCH_DETAIL_BP   = C_IDX++;
                ...
                this.InitSpreadSheet = function () {
                    try {
                        ...
                        ggoSpread.MakeColumns(C_PATCH_DETAIL_BP);
                        ggoSpread.SetMaxRows(0);

                        ggoSpread.SSSetEdit(C_NO, "<%= GetResource("CSTRESX00002") %>", 10, HAlign.Left, 10, CellKind.Protected, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetDate(C_PATCH_DATE, "<%= GetResource("CSTRESX00003") %>", 8, HAlign.Center, CellKind.Protected, false, DateFormat.YYMMDD);
                        ggoSpread.SSSetEdit(C_SEQ, "<%= GetResource("CSTRESX00004") %>", 8, HAlign.Left, 20, CellKind.Protected, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetCombo(C_PATCH_TYPE, "<%= GetResource("CSTRESX00005") %>", 10, HAlign.Center, null, true, "A" + ggoSpread.Source.ColSep + "M");
                        ggoSpread.SSSetCombo(C_PATCH_TYPE_NM, "<%= GetResource("CSTRESX00006") %>", 10, HAlign.Center, CellKind.Protected, null, "<%= GetResource("CSTRESX00017") %>" + vspdData.ColSep + "<%= GetResource("CSTRESX00018") %>");
                        ggoSpread.SSSetEdit(C_PATCH_CONTENT, "<%= GetResource("CSTRESX00007") %>", 70, HAlign.Left, 10, CellKind.Protected, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetEdit(C_PATCH_REMARK, "<%= GetResource("CSTRESX00008") %>", 20, HAlign.Left, 20, CellKind.Protected, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetButton(C_PATCH_DETAIL_BP, true, false);
                        ...
                        ggoSpread.SSSetColHidden(C_NO, C_NO, true);
                ...
                this.InitSpreadSheetCombo = function () {
                    try {
                        lgStrSql = "SELECT MINOR_CD, MINOR_NM_" + lgLang + " MINOR_NM FROM B_MINOR WHERE MAJOR_CD = 'S0013' ORDER BY MINOR_CD ";
                        var result = eni.DbAccess.ExecuteComboDataQuery(lgStrSql);
                        if (result.Value) {
                            if (result.Value) {
                                var cboDataArr = String(result.Value).split(vspdData.RowSep);
                                ggoSpread.Source = vspdData;
                                ggoSpread.SSetComboData(C_PATCH_TYPE, cboDataArr[0]);
                                ggoSpread.SSetComboData(C_PATCH_TYPE_NM, cboDataArr[1]);
                            }
                        }
                    } catch (ex) {
                        eni.Message.DisplayMsgBox(eni.Message.StrBundle("MSGBD00003"), ex.message);
                        ...
                        switch (pCol) {
                            case C_PATCH_REMARK_BP:
                                ...
                                if (arry && arry.Tables && arry.Tables[0].Rows.length > 0) {
                                    ggoSpread.Source = vspdData;
                                    ggoSpread.SetText(C_NO, pRow, arry.Tables[0].Rows[0]["MINOR_CD"]);
                                    ggoSpread.SetText(C_PATCH_REMARK_NM, pRow, arry.Tables[0].Rows[0]["MINOR_NM_<%= lgLang %>"]);
                ...
                this.vspdData_ComboSelChange = function (pSender, pCol, pRow) {
                    try {
                        ...
                        switch (pCol) {
                            case C_PATCH_TYPE: ggoSpread.SetValue(C_PATCH_TYPE_NM, pRow, ggoSpread.GetValue(pCol, pRow)); break;
                            case C_PATCH_TYPE_NM: ggoSpread.SetValue(C_PATCH_TYPE, pRow, ggoSpread.GetValue(pCol, pRow)); break;
                        }
                    } catch (ex) {
                        eni.Message.DisplayMsgBox(eni.Message.StrBundle("MSGBD00003"), ex.message);
                    }
                ...
                this.DbSave = function () {
                    try {
                        ...
                        var tbData = dsData.CreateDataTable("B0002MA1");
                        tbData.AddColumn("DBAccessMode");
                        tbData.AddColumn("PATCH_DATE");
                        tbData.AddColumn("PATCH_TYPE");
                        tbData.AddColumn("PATCH_CONTENT");
                        tbData.AddColumn("PATCH_REMARK");
                        ...
                        for (var i = 1; i < ggoSpread.Source.MaxRows + 1; i++) {
                            switch (ggoSpread.GetText(0, i)) {
                                case ggoSpread.Source.InsertFlag:
                                    ...
                                    nRow.PATCH_DATE = ggoSpread.GetText(C_PATCH_DATE, i, true);
                                    nRow.PATCH_TYPE = ggoSpread.GetText(C_PATCH_TYPE, i, true);
                                    nRow.PATCH_REMARK = ggoSpread.GetText(C_PATCH_REMARK, i, true);
                                    tbData.AddRow(nRow);
                                    break;
                    ...
                    <dx:ContentControl>
                        <eni:eniSplitter ID="tab1Layout" ClientInstanceName="tab1Layout" runat="server" SkinID="BSL">
                            <Panes>
                                ...
                                <dx:SplitterPane Name="tab1Con" Size="32px" MinSize="32px" Visible="false">
                                    <PaneStyle>
