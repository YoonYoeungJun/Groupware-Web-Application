...
namespace eniFramework.WebApp.Modules.B0 {
    ...
    public partial class B0002MA1 : BaseUIACTPage {
        ...
        public override string PageID { get { return "b0002ma1"; } }
        ...
        protected void DbQuery(DatabaseAccess pDbAccess, DbQueryCallBackArgs pArgs, BaseCallbackResultArgs oR_Args) {
            ...
            lgStrSql.AppendLine("SELECT ROW = ROW_NUMBER() OVER (ORDER BY A.NO DESC), ");
            lgStrSql.AppendLine("   	A.NO, ");
            lgStrSql.AppendLine(" 		A.PATCH_DATE, ");
            lgStrSql.AppendLine(" 		CONVERT(INT, RIGHT(A.NO, 5)) SEQ, ");
            lgStrSql.AppendLine(" 		A.PATCH_TYPE, ");
            lgStrSql.AppendLine(string.Format(" 		{0}.UFN_SC_CODENAME('B_MINOR', A.PATCH_TYPE, 'S0013', NULL) PATCH_TYPE_NM, ", Global.gEnvironmonts.Schema));
            lgStrSql.AppendLine(" 		A.PATCH_CONTENT, ");
            lgStrSql.AppendLine(" 		A.PATCH_REMARK ");
            lgStrSql.AppendLine(" FROM S_SYS_MODIFY_HIST_HDR A (NOLOCK) ");
            lgStrSql.AppendLine(") ");
            ...
            var dsB0002MA1 = pDbAccess.ExecuteDataSet(lgStrSql.ToString(), "B0002MA1");
            var dtB0002MA1 = dsB0002MA1.Tables[0];

            for (int i = 0; i < dtB0002MA1.Rows.Count && i < pageRowCount; i++) {
                var row = dtB0002MA1.Rows[i];
                strDataTable.Append(string.Format("{0}{1}", row["NO"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["AUTO_NO_NM"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", ((DateTime)row["PATCH_DATE"]).ToString(CommonVariable.CDT_YYYY_MM_DD), eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["SEQ"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["PATCH_TYPE"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["PATCH_TYPE_NM"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["PATCH_CONTENT"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["PATCH_REMARK"], eniSpread.ColSep));
            ...
            if (dtB0002MA1.Rows.Count > pageRowCount) {
            ...
            for (int i = 0; i < pDbSaveArgs.Data.Rows.Count; i++) {
                ...
                switch ((DBAccessMode)Enum.ToObject(typeof(DBAccessMode), int.Parse(pDbSaveArgs.Data.Rows[i]["DBAccessMode"].ToString()))) {
                    ...
                    case DBAccessMode.Delete:
                        lgStrSql.Clear();
                        lgStrSql.AppendLine(" DELETE S_SYS_MODIFY_HIST_HDR ");
                        lgStrSql.AppendLine(string.Format(" WHERE NO = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["NO"])));
                        lgStrSql.AppendLine("  ");
                        lgStrSql.AppendLine(" DELETE S_SYS_MODIFY_HIST_DTL ");
                        lgStrSql.AppendLine(string.Format(" WHERE NO = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["NO"])));
                        break;
                }
            ...
            try {
                switch (cb_args.Cmd) {
                    ...
                    case "DbSave":
                        ...
                        try {
                            lgDbBatAccess.BatchProcessStart("B0002MA1");
