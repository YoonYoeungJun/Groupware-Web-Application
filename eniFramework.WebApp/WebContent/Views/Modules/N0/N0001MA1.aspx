<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/WebContent/Common/CommonPage.Master" CodeBehind="N0001MA1.aspx.cs" Inherits="eniFramework.WebApp.Modules.N0.N0001MA1" %>
...
<asp:Content ContentPlaceHolderID="Sub_Module_Description" runat="server">
    ...
    *  1. Module Name          : 어드민
    *  2. Function Name        :
    *  3. Program ID           : N0001MA1.aspx
    *  4. Program Name         : 게시판권한관리
    *  5. Program Desc         : 게시판권한관리
    *  6. Comproxy List        :
...
<asp:Content ContentPlaceHolderID="Sub_Script_Holder" runat="server">
    <script type="text/javascript">
        try {
            var N0001MA1 = eni.Initialize(new function () {
                //콜백 아규먼트 클래스
                var DbQueryCallBackArgsClass = function () {
                    this.MENU_ID = new String();
                    this.PARENT_MENU_ID = new String();
                    this.MENU_GROUP = new String();
                    this.MENU_TYPE = new String();
                ...
                this.SetLocalDefaultValue = function () {
                    try {
                        ...
                        cboMenuGroup.SetValue("");
                        cboMenuType.SetValue("");
                ...
                this.InitSpreadSheet = function () {
                    try {
                        ...
                        ggoSpread.SSSetEdit(C_MENU_ID, "메뉴코드", 10, HAlign.Left, 20, CellKind.Protected, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetEdit(C_MENU_NM, "메뉴명", 15, HAlign.Left, 40, CellKind.Required);
                        ggoSpread.SSSetEdit(C_PARENT_MENU_ID, "상위메뉴코드", 10, HAlign.Left, 20, CellKind.Default, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetButton(C_PARENT_MENU_BP, false, false);
                        ggoSpread.SSSetEdit(C_PARENT_MENU_NM, "상위메뉴명", 15, HAlign.Left, 40, CellKind.Protected);
                        ggoSpread.SSSetEdit(C_MENU_DES, "메뉴 설명", 20, HAlign.Left, 100, CellKind.Default);
                        ggoSpread.SSSetCombo(C_MENU_GROUP, "", 10, HAlign.Left, null, true);
                        ggoSpread.SSSetCombo(C_MENU_GROUP_NM, "메뉴그룹", 10, HAlign.Left, CellKind.Protected);
                        ggoSpread.SSSetCombo(C_MENU_TYPE, "", 10, HAlign.Left, null, true);
                        ggoSpread.SSSetCombo(C_MENU_TYPE_NM, "메뉴타입", 8, HAlign.Left, CellKind.Protected);
                        ggoSpread.SSSetEdit(C_MENU_LVL, "메뉴레벨", 7, HAlign.Center, 1, CellKind.Protected);
                        ggoSpread.SSSetEdit(C_MENU_SEQ, "메뉴순서", 7, HAlign.Center, 5, CellKind.Required);
                        ggoSpread.SSSetEdit(C_ROUTE_ID, "라우트코드", 10, HAlign.Left, 20, CellKind.Default, false);
                        ggoSpread.SSSetButton(C_ROUTE_BP, false, false);
                        ggoSpread.SSSetEdit(C_ROUTE_NM, "라우트명", 18, HAlign.Left, 40, CellKind.Protected);
                        ggoSpread.SSSetCheck(C_USE_YN, "사용여부", 7);
                ...
                this.InitSpreadSheetCombo = function () {
                    var strSql = "SELECT MINOR_CD, MINOR_NM FROM S_MINOR WHERE MAJOR_CD = 'S0008' ORDER BY MINOR_CD ";
                    var result = eni.DbAccess.ExecuteComboDataQuery(strSql);
                    if (result.Value) {
                        if (result.Value) {
                            var cboDataArr = String(result.Value).split(vspdData.RowSep);
                            ggoSpread.Source = vspdData;
                            ggoSpread.SSetComboData(C_MENU_GROUP, cboDataArr[0]);
                            ggoSpread.SSetComboData(C_MENU_GROUP_NM, cboDataArr[1]);
                        }
                    }
                    ...
                    ggoSpread.SSetComboText(C_MENU_TYPE_NM, "프로그램");
                    ggoSpread.SSetComboText(C_MENU_TYPE_NM, "메뉴");
                };
                ...
                this.OnFncQuery = function () {
                    try {
                        if (ASPxClientEdit.ValidateGroup("Condition")) {
                            ...
                            var strP1 = cntxtMenuId.GetText();
                            var strP2 = cntxtParentMenuId.GetText();
                            var strP3 = cboMenuGroup.GetValue();
                            var strP4 = cboMenuType.GetValue();
                            var strP5 = cntxtRouteId.GetText();

                            eni.LocalPage.DbQuery(strP1, strP2, strP3, strP4, strP5);
                        } else {
                ...
                this.OnFncInsertRow = function () {
                    try {
                        ...
                        ggoSpread.SpreadRequired(C_MENU_LVL, activeRow);
                    ...
                    <dx:ContentControl>
                        <eni:eniSplitter ID="tab1Layout" ClientInstanceName="tab1Layout" runat="server" SkinID="BSL">
                            <Panes>
                                ...
                                <dx:SplitterPane Name="tab1Con" Size="100">
                                    <ContentCollection>
                                        <dx:SplitterContentControl>
