<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/WebContent/Common/CommonPage.Master" CodeBehind="B3001MA1.aspx.cs" Inherits="eniFramework.WebApp.Modules.B3.B3001MA1" Theme="eniThm" %>
...
<asp:Content ContentPlaceHolderID="Sub_Module_Description" runat="server">
    ...
    *  1. Module Name          : 어드민
    *  2. Function Name        :
    *  3. Program ID           : B3001MA1.aspx
    *  4. Program Name         : 공장정보관리
    *  5. Program Desc         : 공장정보관리
    *  6. Comproxy List        :
...
<asp:Content ContentPlaceHolderID="Sub_Script_Holder" runat="server">
    <script type="text/javascript">
        try {
            var B3001MA1 = eni.Initialize(new function () {
                lgPageRowCount = 100;
                ...
                var C_BIZ_AREA_CD   = C_IDX++,
                    ...
                    C_PLANT_CD      = C_IDX++,
                    C_PLANT_NM      = C_IDX++,
                    C_PLANT_FULL_NM = C_IDX++,
                    C_PLANT_ENG_NM  = C_IDX++,
                    C_PLANT_NM_KO   = C_IDX++,
                    C_PLANT_NM_EN   = C_IDX++,
                    C_PLANT_NM_CN   = C_IDX++,
                    C_PLANT_NM_JP   = C_IDX++,
                ...
                this.InitSpreadSheet = function () {
                    try {
                        ...
                        ggoSpread.MakeColumns(C_SEQ);
                        ggoSpread.SetMaxRows(0);

                        ggoSpread.SSSetCombo(C_BIZ_AREA_CD, "<%= GetResource("CSTRESX00002") %>", 10, null, null, true);
                        ggoSpread.SSSetCombo(C_BIZ_AREA_NM, "<%= GetResource("CSTRESX00003") %>", 16, HAlign.Left, CellKind.Protected);
                        ggoSpread.SSSetEdit(C_PLANT_CD, "<%= GetResource("CSTRESX00004") %>", 8, HAlign.Center, 10, CellKind.Protected);
                        ggoSpread.SSSetEdit(C_PLANT_NM, "<%= GetResource("CSTRESX00005") %>", 15, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_PLANT_FULL_NM, "<%= GetResource("CSTRESX00006") %>", 15, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_PLANT_ENG_NM, "<%= GetResource("CSTRESX00007") %>", 17, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_PLANT_NM_KO, "<%= GetResource("CSTRESX00008") %>", 15, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_PLANT_NM_EN, "<%= GetResource("CSTRESX00009") %>", 15, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_PLANT_NM_CN, "<%= GetResource("CSTRESX00010") %>", 15, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_PLANT_NM_JP, "<%= GetResource("CSTRESX00011") %>", 15, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_SEQ, "<%= GetResource("CSTRESX00012") %>", 8, HAlign.Center, 5, CellKind.Required);

                        ggoSpread.SSSetColHidden(C_SEQ + 1, C_SEQ + 1, true);
                ...
                this.InitSpreadSheetCombo = function () {
                    var strSql = "SELECT BIZ_AREA_CD, BIZ_AREA_NM FROM S_BIZ_AREA ORDER BY BIZ_AREA_CD ";
                ...
                this.OnFncInsertRow = function () {
                    try {
                        ...
                        ggoSpread.SpreadRequired(C_PLANT_CD, activeRow);
                ...
                this.cntxtPlant_PopupButtonClick = function (s, e) {
                    try {
                        ...
                        var popArgs = new CommonWinPopupArgsClass();
                        popArgs.Title = "<%= GetResource("CSTRESX00014")%>";
                        popArgs.AddColumn("PLANT_CD", "<%= GetResource("CSTRESX00004")%>", 10);
                        popArgs.AddColumn("PLANT_NM_<%= lgLang %> PLANT_NM", "<%= GetResource("CSTRESX00005")%>", 15);
                ...
                this.vspdData_ComboSelChange = function (pSender, pCol, pRow) {
                    try {
                        ...
                        switch (pCol) {
                            ...
                            case C_BIZ_AREA_NM: ggoSpread.SetValue(C_BIZ_AREA_CD, pRow, ggoSpread.GetValue(pCol, pRow)); break;
                        }
                    } catch (ex) {
                        eni.Message.DisplayMsgBox(eni.Message.StrBundle("MSGBD00003"), ex.message);
                    }
                ...
                this.DbSave = function () {
                    try {
                        ...
                        var tbData = dsData.CreateDataTable("B3001MA1");
                        tbData.AddColumn("DBAccessMode");
                        tbData.AddColumn("BIZ_AREA_CD");
                        tbData.AddColumn("PLANT_CD");
                        tbData.AddColumn("PLANT_NM");
                        tbData.AddColumn("PLANT_FULL_NM");
                        tbData.AddColumn("PLANT_ENG_NM");
                        tbData.AddColumn("PLANT_NM_KO");
                        tbData.AddColumn("PLANT_NM_EN");
                        tbData.AddColumn("PLANT_NM_CN");
                        tbData.AddColumn("PLANT_NM_JP");
                        ...
                        for (var i = 1; i < ggoSpread.Source.MaxRows + 1; i++) {
                            switch (ggoSpread.GetText(0, i)) {
                                case ggoSpread.Source.InsertFlag:
                                    ...
                                    nRow.PLANT_NM = ggoSpread.GetText(C_PLANT_NM, i, true);
                                    nRow.PLANT_FULL_NM = ggoSpread.GetText(C_PLANT_FULL_NM, i, true);
                                    nRow.PLANT_ENG_NM = ggoSpread.GetText(C_PLANT_ENG_NM, i, true);
                                    nRow.PLANT_NM_KO = ggoSpread.GetText(C_PLANT_NM_KO, i, true);
                                    nRow.PLANT_NM_EN = ggoSpread.GetText(C_PLANT_NM_EN, i, true);
                                    nRow.PLANT_NM_CN = ggoSpread.GetText(C_PLANT_NM_CN, i, true);
                                    nRow.PLANT_NM_JP = ggoSpread.GetText(C_PLANT_NM_JP, i, true);
                                    ...
                                    <ContentCollection>
                                        <dx:SplitterContentControl>
                                            <table class="BSL01TB" cellpadding="0" cellspacing="0">
                                                <tr>
                                                    <th class="TH4C">
                                                        <span class="CAP" onclick="cntxtBizArea.Focus()"><%= GetResource("CSTRESX00013")%></span>
                                                    </th>
                                                    <td class="TD4F">
                                                    ...
                                                    <th class="TH4C">
                                                        <span class="CAP" onclick="cntxtPlant.Focus()"><%= GetResource("CSTRESX00014")%></span>
                                                    </th>
                                                    <td class="TD4F">
