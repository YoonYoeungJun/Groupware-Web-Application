<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/WebContent/Common/CommonPage.Master" CodeBehind="B2004MA1.aspx.cs" Inherits="eniFramework.WebApp.Modules.B2.B2004MA1" %>
...
<asp:Content ContentPlaceHolderID="Sub_Module_Description" runat="server">
    ...
    *  1. Module Name          : 어드민
    *  2. Function Name        :
    *  3. Program ID           : B2004MA1.aspx
    *  4. Program Name         : 시스템권한상세정보관리
    *  5. Program Desc         : 시스템권한상세정보관리
    *  6. Comproxy List        :
    *  7. Modified date(First) : 2014-07-25
    *  8. Modified date(Last)  : 2016-04-19
...
<asp:Content ContentPlaceHolderID="Sub_ScriptIntelliSense_Holder" runat="server">
    <script type="text/javascript">
        ...
        cntxtUserRole       = ASPxClientTextBox.Cast();
        cntxtUserRoleName   = ASPxClientTextBox.Cast();
...
<asp:Content ContentPlaceHolderID="Sub_Script_Holder" runat="server">
    <script type="text/javascript">
        try {
            var B2004MA1 = eni.Initialize(new function () {
                lgPageRowCount = 100;
                ...
                this.SetLocalDefaultValue = function () {
                    try {
                        eni.SetToolbar("1101110001000000");

                        cntxtUserRole.SetText("");
                        cntxtUserRoleName.SetText("");
                ...
                this.InitSpreadSheet = function () {
                    try {
                        ...
                        ggoSpread.SSSetEdit(C_USR_ROLE_ID, "<%= GetResource("CSTRESX00003") %>", 10, HAlign.Left, 20, CellKind.Protected, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetEdit(C_MENU_ID, "<%= GetResource("CSTRESX00004") %>", 10, HAlign.Left, 50, CellKind.Protected);
                        ggoSpread.SSSetEdit(C_MENU_NM, "<%= GetResource("CSTRESX00005") %>", 30, HAlign.Left, 50, CellKind.Protected, false, EditCharCase.NoCase);
                        ggoSpread.SSSetEdit(C_MENU_TYPE_NM, "<%= GetResource("CSTRESX00006") %>", 7, HAlign.Left, 50, CellKind.Protected, false, EditCharCase.NoCase);
                        ggoSpread.SSSetCheck(C_USE_YN, "<%= GetResource("CSTRESX00007") %>", 7);

                        ggoSpread.SSSetColHidden(C_USR_ROLE_ID, C_USR_ROLE_ID, true);
                ...
                this.OnFncQuery = function () {
                    try {
                        if (ASPxClientEdit.ValidateGroup("Condition")) {
                            ...
                            var strP1 = cntxtUserRole.GetText();
                ...
                this.cntxtUserRole_CodeBoxValueChanged = function (s, e) {
                    try {
                        ...
                        if (strCode) {
                            ...
                            if (!result.Code) {
                                if (result.Value) {
                                    cntxtUserRoleName.SetText(result.Value);
                                } else {
                                    s.SetText("");
                                    cntxtUserRoleName.SetText("");
                ...
                this.cntxtUserRole_PopupButtonClick = function (s, e) {
                    try {
                        var popArgs = new CommonWinPopupArgsClass();
                        popArgs.Title = "<%= GetResource("CSTRESX00010") %>";
                        popArgs.AddColumn("USR_ROLE_ID", "<%= GetResource("CSTRESX00011") %>", 10);
                        popArgs.AddColumn("USR_ROLE_NM_" + lgLang, "<%= GetResource("CSTRESX00012") %>", 15);
                        ...
                        if (arry && arry.Tables && arry.Tables[0].Rows.length > 0) {
                            cntxtUserRole.SetText(arry.Tables[0].Rows[0]["USR_ROLE_ID"]);
                            cntxtUserRoleName.SetText(arry.Tables[0].Rows[0]["USR_ROLE_NM_" + lgLang]);
                ...
                this.DbSave = function () {
                    try {
                        ...
                        var tbData = dsData.CreateDataTable("B2004MA1");
                        tbData.AddColumn("DBAccessMode");
                        tbData.AddColumn("USR_ROLE_ID");
                        tbData.AddColumn("MENU_ID");
                        tbData.AddColumn("USE_YN");

                        var strRoleId = cntxtUserRole.GetText();

                        ggoSpread.Source = vspdData;
                        for (var i = 1; i < ggoSpread.Source.MaxRows + 1; i++) {
                            if (strRoleId !== ggoSpread.GetText(C_USR_ROLE_ID, i, true)) {
                                throw eni.Message.StrBundle("MSGBD00117");
                            }
                            ...
                            switch (ggoSpread.GetText(0, i)) {
                                case ggoSpread.Source.InsertFlag:
                                    ...
                                    nRow.USE_YN = ggoSpread.GetText(C_USE_YN, i, true);
                                    tbData.AddRow(nRow);
                                    break;
...
<asp:Content ContentPlaceHolderID="Sub_Content_Form_Holder" runat="server">
    ...
    <eni:eniPageControl ID="tabMain" ClientInstanceName="tabMain" SkinID="BSL" runat="server">
        <TabPages>
            <dx:TabPage Text="시스템권한상세정보관리">
                <ContentCollection>
                    <dx:ContentControl>
                        <eni:eniSplitter ID="tab1Layout" ClientInstanceName="tab1Layout" runat="server" SkinID="BSL">
                            <Panes>
                                <dx:SplitterPane Name="tab1Top" Size="30px" MaxSize="30px">
                                    <ContentCollection>
                                        <dx:SplitterContentControl>
                                            <span style="color:red">(개발중)미완성</span>
                                        </dx:SplitterContentControl>
                                    </ContentCollection>
                                </dx:SplitterPane>
                                    ...
                                    <ContentCollection>
                                        <dx:SplitterContentControl>
                                            <table class="BSL01TB" cellpadding="0" cellspacing="0">
                                                <tr>
                                                    <th class="TH4C">
                                                        <span class="CAP" onclick="cntxtUserRole.Focus()"><%= GetResource("CSTRESX00003")%></span>
                                                    </th>
                                                    <td class="TD4F">
                                                        <eni:eniCodeNameBox ID="cntxtUserRole" ClientInstanceName="cntxtUserRole" runat="server" SkinID="BSL" EnterActionQuery="true"
                                                            CodeBoxWidth="100px" NameBoxWidth="150px" NameBoxClientReadOnly="true" CodeBoxUpperCase="true" NameBoxUpperCase="true">
                                                            <CodeNameClientSideEvents CodeBoxValueChanged="eni.LocalPage.cntxtUserRole_CodeBoxValueChanged" 
                                                                                    PopupButtonClick="eni.LocalPage.cntxtUserRole_PopupButtonClick" />
                                                            <ValidationSettings ValidationGroup="Condition" RequiredField-IsRequired="true"></ValidationSettings>
                                                        </eni:eniCodeNameBox>
                                                    </td>
                                                    <th class="TH4C">
                                                        <span class="CAP"></span>
