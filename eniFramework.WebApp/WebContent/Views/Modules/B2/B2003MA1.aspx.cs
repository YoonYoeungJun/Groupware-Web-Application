...
namespace eniFramework.WebApp.Modules.B2 {
    ...
    public partial class B2003MA1 : BaseUIACTPage {
        ...
        public override string PageID { get { return "b2003ma1"; } }
        ...
        protected void DbQuery(DatabaseAccess pDbAccess, DbQueryCallBackArgs pArgs, BaseCallbackResultArgs oR_Args) {
            ...
            lgStrSql.AppendLine("SELECT ROW = ROW_NUMBER() OVER (ORDER BY A.USR_ROLE_ID), ");
            lgStrSql.AppendLine("     A.USR_ROLE_ID, ");
            lgStrSql.AppendLine("     A.USR_ROLE_NM, ");
            lgStrSql.AppendLine("     A.USR_ROLE_NM_KO, ");
            lgStrSql.AppendLine("     A.USR_ROLE_NM_EN, ");
            lgStrSql.AppendLine("     A.USR_ROLE_NM_CN, ");
            lgStrSql.AppendLine("     A.USR_ROLE_NM_JP, ");
            ...
            lgStrSql.AppendLine(" FROM S_USR_ROLE A (NOLOCK) ");
            lgStrSql.AppendLine(string.Format("WHERE A.USR_ROLE_ID LIKE {0}", Data.FilterVariable(pArgs.Data.USR_ROLE_ID, FilterVarCondition.AllLikeCondition)));
            lgStrSql.AppendLine(") ");
            ...
            var dsB2003MA1 = pDbAccess.ExecuteDataSet(lgStrSql.ToString(), "B2003MA1");
            var dtB2003MA1 = dsB2003MA1.Tables[0];

            for (int i = 0; i < dtB2003MA1.Rows.Count && i < pageRowCount; i++) {
                var row = dtB2003MA1.Rows[i];
            ...
            if (dtB2003MA1.Rows.Count > pageRowCount) {
            ...
            try {
                switch(cb_args.Cmd) {
                    ...
                    case "DbSave":
                        ...
                        try {
                            lgDbBatAccess.BatchProcessStart("B2003MA1");
