...
namespace eniFramework.WebApp.Modules.B1 {
    ...
    public partial class B1000MA1 : BaseUIACTPage {
        ...
        public override string PageID { get { return "b1000ma1"; } }
        ...
        protected void DbQuery(DatabaseAccess pDbAccess, DbQueryCallBackArgs pArgs, BaseCallbackResultArgs oR_Args) {
            ...
            lgStrSql.AppendLine("SELECT ROW = ROW_NUMBER() OVER (ORDER BY MAJOR_CD, MAJOR_NM), ");
            lgStrSql.AppendLine(" MAJOR_CD, ");
            lgStrSql.AppendLine(" MAJOR_NM, ");
            lgStrSql.AppendLine(" MAJOR_NM_KO, ");
            lgStrSql.AppendLine(" MAJOR_NM_EN, ");
            lgStrSql.AppendLine(" MAJOR_NM_CN, ");
            lgStrSql.AppendLine(" MAJOR_NM_JP, ");
            lgStrSql.AppendLine(" MINOR_LEN, ");
            lgStrSql.AppendLine(" MAJOR_TYPE, ");
            lgStrSql.AppendLine(string.Format(" CASE WHEN MAJOR_TYPE = 'S' THEN N'{0}' ELSE N'{1}' END MAJOR_TYPE_NM, ", GetResource("CSTRESX00011"), GetResource("CSTRESX00012")));
            ...
            lgStrSql.AppendLine(" FROM B_MAJOR (NOLOCK) ");
            ...
            var dsB1000MA1 = pDbAccess.ExecuteDataSet(lgStrSql.ToString(), "B1000MA1");
            var dtB1000MA1 = dsB1000MA1.Tables[0];

            for (int i = 0; i < dtB1000MA1.Rows.Count && i < pageRowCount; i++) {
                var row = dtB1000MA1.Rows[i];
                strDataTable.Append(string.Format("{0}{1}", row["MAJOR_CD"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["MAJOR_NM"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["MAJOR_NM_KO"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["MAJOR_NM_EN"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["MAJOR_NM_CN"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["MAJOR_NM_JP"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["MINOR_LEN"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["MAJOR_TYPE"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["MAJOR_TYPE_NM"], eniSpread.ColSep));
            ...
            if (dtB1000MA1.Rows.Count > pageRowCount) {
            ...
            for (int i = 0; i < pDbSaveArgs.Data.Rows.Count; i++) {
                ...
                switch ((DBAccessMode)Enum.ToObject(typeof(DBAccessMode), int.Parse(pDbSaveArgs.Data.Rows[i]["DBAccessMode"].ToString()))) {
                    case DBAccessMode.Create:
                        ...
                        if (rowCount > 0) {
                            throw new ExceptionManager(string.Format("필드 : AUTO_NO_TYPE, 값 : {0}<br>필드 : EFFECT_FROM_DT, 값 : {1}은 이미 존재하는 데이터 입니다.", pDbSaveArgs.Data.Rows[i]["AUTO_NO_TYPE"], Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["EFFECT_FROM_DT"])));
                        }

                        lgStrSql.Clear();
                        lgStrSql.AppendLine(" INSERT INTO B_MAJOR (MAJOR_CD, MAJOR_NM, MAJOR_NM_KO, MAJOR_NM_EN, MAJOR_NM_CN, MAJOR_NM_JP, MINOR_LEN, MAJOR_TYPE, USE_YN, INSRT_USER_ID, INSRT_DT, UPDT_USER_ID, UPDT_DT, OTL) ");
                        lgStrSql.Append(" VALUES( ");
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_CD"], true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM_KO"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM_EN"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM_CN"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM_JP"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MINOR_LEN"],  FilterVarType.NoBraceButReplSingleWithDoubleQuotation, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_TYPE"], true));
                    ...
                    case DBAccessMode.Update:
                        ...
                        lgStrSql.AppendLine(string.Format(" MAJOR_NM = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM"], true, true)));
                        lgStrSql.AppendLine(string.Format(" MAJOR_NM_KO = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM_KO"], true, true)));
                        lgStrSql.AppendLine(string.Format(" MAJOR_NM_EN = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM_EN"], true, true)));
                        lgStrSql.AppendLine(string.Format(" MAJOR_NM_CN = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM_CN"], true, true)));
                        lgStrSql.AppendLine(string.Format(" MAJOR_NM_JP = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_NM_JP"], true, true)));
                        lgStrSql.AppendLine(string.Format(" MINOR_LEN = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MINOR_LEN"], FilterVarType.NoBraceButReplSingleWithDoubleQuotation, true)));
                        lgStrSql.AppendLine(string.Format(" MAJOR_TYPE = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_TYPE"], true)));
                        ...
                        lgStrSql.AppendLine(" FROM B_MAJOR A ");
                        lgStrSql.AppendLine(string.Format(" WHERE A.MAJOR_CD = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_CD"])));

                        break;
                    case DBAccessMode.Delete:

                        lgStrSql.Clear();
                        lgStrSql.AppendLine(" DELETE B_MAJOR ");
                        lgStrSql.AppendLine(string.Format(" WHERE MAJOR_CD = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["MAJOR_CD"])));
            ...
            try {
                switch(cb_args.Cmd) {
                    ...
                    case "DbSave":
                        ...
                        try {
                            lgDbBatAccess.BatchProcessStart("B1000MA1");
