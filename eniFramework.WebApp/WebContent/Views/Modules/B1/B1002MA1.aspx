<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/WebContent/Common/CommonPage.Master" CodeBehind="B1002MA1.aspx.cs" Inherits="eniFramework.WebApp.Modules.B1.B1002MA1" %>
...
<asp:Content ContentPlaceHolderID="Sub_ScriptIntelliSense_Holder" runat="server">
    <script type="text/javascript">
        ...
        cntxtMajor          = ASPxClientTextBox.Cast();
        cntxtMajorName      = ASPxClientTextBox.Cast();
...
<asp:Content ContentPlaceHolderID="Sub_Script_Holder" runat="server">
    <script type="text/javascript">
        try {
            var B1002MA1 = eni.Initialize(new function () {
                lgPageRowCount = 100;

                var C_IDX = 1;
                var C_MAJOR_CD      = C_IDX++,
                    C_MINOR_CD      = C_IDX++,
                    C_SEQ_NO        = C_IDX++,
                    C_REFERENCE     = C_IDX++,
                    C_REFERENCE_KO  = C_IDX++,
                    C_REFERENCE_EN  = C_IDX++,
                    C_REFERENCE_CN  = C_IDX++,
                    C_REFERENCE_JP  = C_IDX++,
                    C_REF_TYPE      = C_IDX++,
                    C_REF_TYPE_NM   = C_IDX++;
                ...
                this.InitSpreadSheet = function () {
                    try {
                        ...
                        ggoSpread.MakeColumns(C_REF_TYPE_NM);
                        ggoSpread.SetMaxRows(0);

                        ggoSpread.SSSetEdit(C_MAJOR_CD, "<%= GetResource("CSTRESX00002") %>", 12, HAlign.Center, 5, CellKind.Protected, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetEdit(C_MINOR_CD, "<%= GetResource("CSTRESX00003") %>", 12, HAlign.Left, 5, CellKind.Protected, false, EditCharCase.UpperCase);
                        ggoSpread.SSSetEdit(C_SEQ_NO, "<%= GetResource("CSTRESX00004") %>", 20, HAlign.Left, 5, CellKind.Required);
                        ggoSpread.SSSetEdit(C_REFERENCE, "<%= GetResource("CSTRESX00005") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_REFERENCE_KO, "<%= GetResource("CSTRESX00006") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_REFERENCE_EN, "<%= GetResource("CSTRESX00007") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_REFERENCE_CN, "<%= GetResource("CSTRESX00008") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_REFERENCE_JP, "<%= GetResource("CSTRESX00009") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetCombo(C_REF_TYPE, "", 10, HAlign.Center, null, true, "U" + vspdData.ColSep + "S");
                        ggoSpread.SSSetCombo(C_REF_TYPE_NM, "<%= GetResource("CSTRESX00010") %>", 15, HAlign.Center, CellKind.Protected, false, "<%= GetResource("CSTRESX00011") %>" + vspdData.ColSep + "<%= GetResource("CSTRESX00012") %>");
                ...
                this.OnFncInsertRow = function () {
                    try {
                        ...
                        ggoSpread.SpreadRequired(C_REF_TYPE_NM, activeRow);

                        ggoSpread.SetText(C_MAJOR_CD, activeRow, cntxtMajor.GetText());
                        ggoSpread.SetValue(C_REF_TYPE, activeRow, 0);
                        ggoSpread.SetValue(C_REF_TYPE_NM, activeRow, 0);
                ...
                this.OnFncCopyRow = function () {
                    try {
                        ...
                        if (ggoSpread.Source.MaxRows == 0) {
                            ...
                            ggoSpread.SpreadRequired(C_MAJOR_CD, copy_row, C_REFERENCE_JP, copy_row);
                            ggoSpread.SpreadRequired(C_REF_TYPE_NM, copy_row);

                            ggoSpread.SetText(C_MAJOR_CD, copy_row, ggoSpread.GetText(C_MAJOR_CD, active_row, false));
                            ggoSpread.SetText(C_REFERENCE, copy_row, ggoSpread.GetText(C_REFERENCE, active_row, false));
                            ggoSpread.SetText(C_REFERENCE_KO, copy_row, ggoSpread.GetText(C_REFERENCE_KO, active_row, false));
                            ggoSpread.SetText(C_REFERENCE_EN, copy_row, ggoSpread.GetText(C_REFERENCE_EN, active_row, false));
                            ggoSpread.SetText(C_REFERENCE_CN, copy_row, ggoSpread.GetText(C_REFERENCE_CN, active_row, false));
                            ggoSpread.SetText(C_REFERENCE_JP, copy_row, ggoSpread.GetText(C_REFERENCE_JP, active_row, false));
                            ggoSpread.SetText(C_REF_TYPE, copy_row, ggoSpread.GetText(C_REF_TYPE, active_row, false));
                            ggoSpread.SetText(C_REF_TYPE_NM, copy_row, ggoSpread.GetText(C_REF_TYPE_NM, active_row, false));

                            ggoSpread.SetText(C_MINOR_CD, copy_row, "");
                ...
                this.cntxtMajor_CodeBoxValueChanged = function (s, e) {
                    try {
                        var strCode = s.GetText();
                        if (strCode) {
                            var result = eni.DbAccess.ExecuteCodeName("B_MAJOR", strCode);
                            if (!result.Code) {
                                if (result.Value) {
                                    cntxtMajorName.SetText(result.Value);
                                } else {
                                    s.SetText("");
                                    cntxtMajorName.SetText("");
                                    eni.Message.DisplayBundleMsgBox("MSGBD00001", "MSGBD00111", null, null, function () {
                                        cntxtMajorName.Focus();
                                    });
                                }
                            } else {
                ...
                this.cntxtMajor_PopupButtonClick = function (s, e) {
                    try {
                        var popArgs = new CommonWinPopupArgsClass();
                        popArgs.Title = "<%= GetResource("CSTRESX00002") %>";
                        popArgs.AddColumn("MAJOR_CD", "<%= GetResource("CSTRESX00002") %>", 10);
                        popArgs.AddColumn("MAJOR_NM_<%= lgLang %>", "<%= GetResource("CSTRESX00013") %>", 15);
                        popArgs.From = "B_MAJOR";
                        ...
                        if (arry && arry.Tables && arry.Tables[0].Rows.length > 0) {
                            cntxtMajor.SetText(arry.Tables[0].Rows[0]["MAJOR_CD"]);
                            cntxtMajorName.SetText(arry.Tables[0].Rows[0]["MAJOR_NM_<%= lgLang %>"]);
                        }
                    } catch (ex) {
                        eni.Message.DisplayMsgBox(eni.Message.StrBundle("MSGBD00003"), ex.message);
                    }
                ...
                this.vspdData_ComboSelChange = function (pSender, pCol, pRow) {
                    try {
                        ...
                        switch (pCol) {
                            case C_REF_TYPE: ggoSpread.SetValue(C_REF_TYPE_NM, pRow, ggoSpread.GetValue(pCol, pRow)); break;
                            case C_REF_TYPE_NM: ggoSpread.SetValue(C_REF_TYPE, pRow, ggoSpread.GetValue(pCol, pRow)); break;
                        }
                    } catch (ex) {
                        eni.Message.DisplayMsgBox(eni.Message.StrBundle("MSGBD00003"), ex.message);
                    }
                ...
                this.DbSave = function () {
                    try {
                        ...
                        var tbData = dsData.CreateDataTable("B1002MA1");
                        tbData.AddColumn("DBAccessMode");
                        tbData.AddColumn("MAJOR_CD");
                        tbData.AddColumn("MINOR_CD");
                        tbData.AddColumn("SEQ_NO");
                        tbData.AddColumn("REFERENCE");
                        tbData.AddColumn("REFERENCE_KO");
                        tbData.AddColumn("REFERENCE_EN");
                        tbData.AddColumn("REFERENCE_CN");
                        tbData.AddColumn("REFERENCE_JP");
                        tbData.AddColumn("REF_TYPE");
                        ...
                        for (var i = 1; i < ggoSpread.Source.MaxRows + 1; i++) {
                            switch (ggoSpread.GetText(0, i)) {
                                case ggoSpread.Source.InsertFlag:
                                    ...
                                    nRow.MAJOR_CD = ggoSpread.GetText(C_MAJOR_CD, i, true);
                                    nRow.MINOR_CD = ggoSpread.GetText(C_MINOR_CD, i, true);
                                    nRow.SEQ_NO = ggoSpread.GetText(C_SEQ_NO, i, true);
                                    nRow.REFERENCE = ggoSpread.GetText(C_REFERENCE, i, true);
                                    nRow.REFERENCE_KO = ggoSpread.GetText(C_REFERENCE_KO, i, true);
                                    nRow.REFERENCE_EN = ggoSpread.GetText(C_REFERENCE_EN, i, true);
                                    nRow.REFERENCE_CN = ggoSpread.GetText(C_REFERENCE_CN, i, true);
                                    nRow.REFERENCE_JP = ggoSpread.GetText(C_REFERENCE_JP, i, true);
                                    nRow.REF_TYPE = ggoSpread.GetText(C_REF_TYPE, i, true);
                                    tbData.AddRow(nRow);
                                    break;
                                    ...
                                    <ContentCollection>
                                        <dx:SplitterContentControl>
                                            <table class="BSL01TB" cellpadding="0" cellspacing="0">
                                                <tr>
                                                    <th class="TH4C">
                                                        <span class="CAP" onclick="cntxtMajor.Focus()"><%= GetResource("CSTRESX00002")%></span>
                                                    </th>
                                                    <td class="TD4F">
                                                        <eni:eniCodeNameBox ID="cntxtMajor" ClientInstanceName="cntxtMajor" runat="server" SkinID="BSL" EnterActionQuery="true"
                                                            CodeBoxWidth="100px" NameBoxWidth="150px" NameBoxClientReadOnly="true" CodeBoxUpperCase="true" NameBoxUpperCase="true">
                                                            <CodeNameClientSideEvents CodeBoxValueChanged="eni.LocalPage.cntxtMajor_CodeBoxValueChanged" 
                                                                                    PopupButtonClick="eni.LocalPage.cntxtMajor_PopupButtonClick" />
                                                        </eni:eniCodeNameBox>
                                                    </td>
                                                    <th class="TH4C">
                                                        <span class="CAP"></span>
