<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/WebContent/Common/CommonPage.Master" CodeBehind="B1000MA1.aspx.cs" Inherits="eniFramework.WebApp.Modules.B1.B1000MA1" %>
...
<asp:Content ContentPlaceHolderID="Sub_Module_Description" runat="server">
    ...
    *  1. Module Name          : 시스템설정
    *  2. Function Name        :
    *  3. Program ID           : B1000MA1.aspx
    *  4. Program Name         : MAJOR 코드 등록
    *  5. Program Desc         : MAJOR 코드 등록
    *  6. Comproxy List        :
...
<asp:Content ContentPlaceHolderID="Sub_Script_Holder" runat="server">
    <script type="text/javascript">
        try {
            var B1000MA1 = eni.Initialize(new function () {
                lgPageRowCount = 100;
                ...
                this.InitSpreadSheet = function () {
                    try {
                        ...
                        ggoSpread.SSSetEdit(C_MAJOR_NM, "<%= GetResource("CSTRESX00003") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_MAJOR_NM_KO, "<%= GetResource("CSTRESX00004") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_MAJOR_NM_EN, "<%= GetResource("CSTRESX00005") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_MAJOR_NM_CN, "<%= GetResource("CSTRESX00006") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetEdit(C_MAJOR_NM_JP, "<%= GetResource("CSTRESX00007") %>", 20, HAlign.Left, 50, CellKind.Required);
                        ggoSpread.SSSetFloat(C_MINOR_LEN, "<%= GetResource("CSTRESX00008") %>", 15, HAlign.Right, CellKind.Required, false, ",", ".", 0, null, CurrencyLeadingZero.LeadingZeroYes, 0, 10);
                        ggoSpread.SSSetCombo(C_MAJOR_TYPE, "", 10, HAlign.Center, null, true, "U" + vspdData.ColSep + "S");
                        ggoSpread.SSSetCombo(C_MAJOR_TYPE_NM, "<%= GetResource("CSTRESX00009") %>", 15, HAlign.Center, CellKind.Protected, false, "<%= GetResource("CSTRESX00011") %>" + vspdData.ColSep + "<%= GetResource("CSTRESX00012") %>");
                        ggoSpread.SSSetCheck(C_USE_YN, "<%= GetResource("CSTRESX00010") %>", 8);
                ...
                this.OnFncInsertRow = function () {
                    try {
                        ...
                        ggoSpread.SpreadRequired(C_MAJOR_CD, activeRow);
                        ggoSpread.SpreadRequired(C_MAJOR_TYPE_NM, activeRow);

                        ggoSpread.SetValue(C_MINOR_LEN, activeRow, 5);
                        ggoSpread.SetValue(C_MAJOR_TYPE, activeRow, 0);
                        ggoSpread.SetValue(C_MAJOR_TYPE_NM, activeRow, 0);
                ...
                this.OnFncCopyRow = function () {
                    try {
                        ...
                        if (ggoSpread.Source.MaxRows == 0) {
                            ...
                            ggoSpread.SpreadRequired(C_MAJOR_CD, copy_row, C_MINOR_LEN, copy_row);
                            ggoSpread.SpreadRequired(C_MAJOR_TYPE_NM, copy_row);
                            
                            ggoSpread.SetText(C_MAJOR_CD, copy_row, "");
                            ggoSpread.SetText(C_MAJOR_NM, copy_row, ggoSpread.GetText(C_MAJOR_NM, active_row, false));
                            ggoSpread.SetText(C_MAJOR_NM_KO, copy_row, ggoSpread.GetText(C_MAJOR_NM_KO, active_row, false));
                            ggoSpread.SetText(C_MAJOR_NM_EN, copy_row, ggoSpread.GetText(C_MAJOR_NM_EN, active_row, false));
                            ggoSpread.SetText(C_MAJOR_NM_CN, copy_row, ggoSpread.GetText(C_MAJOR_NM_CN, active_row, false));
                            ggoSpread.SetText(C_MAJOR_NM_JP, copy_row, ggoSpread.GetText(C_MAJOR_NM_JP, active_row, false));
                            ggoSpread.SetText(C_MINOR_LEN, copy_row, ggoSpread.GetText(C_MINOR_LEN, active_row, false));

                            ggoSpread.SetValue(C_MAJOR_TYPE, copy_row, 0);
                            ggoSpread.SetValue(C_MAJOR_TYPE_NM, copy_row, 0);
                            ggoSpread.SetValue(C_USE_YN, copy_row, 1);
                ...
                this.cntxtMajor_PopupButtonClick = function (s, e) {
                    try {
                        ...
                        popArgs.AddColumn("MAJOR_NM_<%= lgLang %>", "<%= GetResource("CSTRESX00003") %>", 15);
                ...
                this.vspdData_ComboSelChange = function (pSender, pCol, pRow) {
                    try {
                        ...
                        switch (pCol) {
                            case C_MAJOR_TYPE: ggoSpread.SetValue(C_MAJOR_TYPE_NM, pRow, ggoSpread.GetValue(pCol, pRow)); break;
                            case C_MAJOR_TYPE_NM: ggoSpread.SetValue(C_MAJOR_TYPE, pRow, ggoSpread.GetValue(pCol, pRow)); break;
                        }
                    } catch (ex) {
                        eni.Message.DisplayMsgBox(eni.Message.StrBundle("MSGBD00003"), ex.message);
                    }
                ...
                this.DbSave = function () {
                    try {
                        ...
                        var tbData = dsData.CreateDataTable("B1000MA1");
                        tbData.AddColumn("DBAccessMode");
                        tbData.AddColumn("MAJOR_CD");
                        tbData.AddColumn("MAJOR_NM");
                        tbData.AddColumn("MAJOR_NM_KO");
                        tbData.AddColumn("MAJOR_NM_EN");
                        tbData.AddColumn("MAJOR_NM_CN");
                        tbData.AddColumn("MAJOR_NM_JP");
                        tbData.AddColumn("MINOR_LEN");
                        tbData.AddColumn("MAJOR_TYPE");
                        ...
                        for (var i = 1; i < ggoSpread.Source.MaxRows + 1; i++) {
                            switch (ggoSpread.GetText(0, i)) {
                                case ggoSpread.Source.InsertFlag:
                                    ...
                                    nRow.MAJOR_NM = ggoSpread.GetText(C_MAJOR_NM, i, true);
                                    nRow.MAJOR_NM_KO = ggoSpread.GetText(C_MAJOR_NM_KO, i, true);
                                    nRow.MAJOR_NM_EN = ggoSpread.GetText(C_MAJOR_NM_EN, i, true);
                                    nRow.MAJOR_NM_CN = ggoSpread.GetText(C_MAJOR_NM_CN, i, true);
                                    nRow.MAJOR_NM_JP = ggoSpread.GetText(C_MAJOR_NM_JP, i, true);
                                    nRow.MINOR_LEN = ggoSpread.GetText(C_MINOR_LEN, i, true);
                                    nRow.MAJOR_TYPE = ggoSpread.GetText(C_MAJOR_TYPE, i, true);
                                    ...
                                    <ContentCollection>
                                        <dx:SplitterContentControl>
                                            <table class="BSL01TB" cellpadding="0" cellspacing="0">
                                                <tr>
                                                    <th class="TH4C">
                                                        <span class="CAP" onclick="cntxtMajor.Focus()"><%= GetResource("CSTRESX00001")%></span>
                                                    </th>
                                                    <td class="TD4F">
