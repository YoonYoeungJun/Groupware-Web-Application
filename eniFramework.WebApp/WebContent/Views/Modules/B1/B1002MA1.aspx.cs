...
namespace eniFramework.WebApp.Modules.B1 {
    ...
    public partial class B1002MA1 : BaseUIACTPage {
        ...
        public override string PageID { get { return "b1002ma1"; } }
        ...
        protected void DbQuery(DatabaseAccess pDbAccess, DbQueryCallBackArgs pArgs, BaseCallbackResultArgs oR_Args) {
            ...
            lgStrSql.AppendLine("SELECT ROW = ROW_NUMBER() OVER (ORDER BY MAJOR_CD, MINOR_CD, SEQ_NO, REFERENCE), ");
            lgStrSql.AppendLine(" MAJOR_CD, ");
            lgStrSql.AppendLine(" MINOR_CD, ");
            lgStrSql.AppendLine(" SEQ_NO, ");
            lgStrSql.AppendLine(" REFERENCE, ");
            lgStrSql.AppendLine(" REFERENCE_KO, ");
            lgStrSql.AppendLine(" REFERENCE_EN, ");
            lgStrSql.AppendLine(" REFERENCE_CN, ");
            lgStrSql.AppendLine(" REFERENCE_JP, ");
            lgStrSql.AppendLine(" REF_TYPE, ");
            lgStrSql.AppendLine(string.Format(" CASE WHEN REF_TYPE = 'S' THEN N'{0}' ELSE N'{1}' END REF_TYPE_NM, ", GetResource("CSTRESX00011"), GetResource("CSTRESX00012")));
            ...
            lgStrSql.AppendLine(" FROM S_CONFIGURATION (NOLOCK) ");
            ...
            var dsB1002MA1 = pDbAccess.ExecuteDataSet(lgStrSql.ToString(), "B1002MA1");
            var dtB1002MA1 = dsB1002MA1.Tables[0];

            for (int i = 0; i < dtB1002MA1.Rows.Count && i < pageRowCount; i++) {
                var row = dtB1002MA1.Rows[i];
                ...
                strDataTable.Append(string.Format("{0}{1}", row["SEQ_NO"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["REFERENCE"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["REFERENCE_KO"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["REFERENCE_EN"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["REFERENCE_CN"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["REFERENCE_JP"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["REF_TYPE"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", row["REF_TYPE_NM"], eniSpread.ColSep));
                strDataTable.Append(string.Format("{0}{1}", i + ((curPage - 1) * pageRowCount) + 1, eniSpread.RowSep));
            }

            if (dtB1002MA1.Rows.Count > pageRowCount) {
            ...
            for (int i = 0; i < pDbSaveArgs.Data.Rows.Count; i++) {
                ...
                switch ((DBAccessMode)Enum.ToObject(typeof(DBAccessMode), int.Parse(pDbSaveArgs.Data.Rows[i]["DBAccessMode"].ToString()))) {
                    case DBAccessMode.Create:
                        ...
                        lgStrSql.Clear();
                        lgStrSql.AppendLine(" INSERT INTO S_CONFIGURATION (MINOR_CD, MAJOR_CD, REFERENCE, REFERENCE_KO, REFERENCE_EN, REFERENCE_CN, REFERENCE_JP, REF_TYPE, INSRT_USER_ID, INSRT_DT, UPDT_USER_ID, UPDT_DT, OTL) ");
                        ...
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["SEQ_NO"], true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE_KO"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE_EN"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE_CN"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE_JP"], true, true));
                        lgStrSql.Append(Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REF_TYPE"], true));
                    ...
                    case DBAccessMode.Update:
                        ...
                        lgStrSql.AppendLine(string.Format(" SEQ_NO = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["SEQ_NO"], true, true)));
                        lgStrSql.AppendLine(string.Format(" REFERENCE = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE"], true, true)));
                        lgStrSql.AppendLine(string.Format(" REFERENCE_KO = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE_KO"], true, true)));
                        lgStrSql.AppendLine(string.Format(" REFERENCE_EN = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE_EN"], true, true)));
                        lgStrSql.AppendLine(string.Format(" REFERENCE_CN = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE_CN"], true, true)));
                        lgStrSql.AppendLine(string.Format(" REFERENCE_JP = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REFERENCE_JP"], true, true)));
                        lgStrSql.AppendLine(string.Format(" REF_TYPE = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["REF_TYPE"], true)));
                        ...
                        lgStrSql.AppendLine(" FROM S_CONFIGURATION A ");
                        ...
                        lgStrSql.AppendLine(string.Format(" AND A.SEQ_NO = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["SEQ_NO"])));

                        break;
                    case DBAccessMode.Delete:

                        lgStrSql.Clear();
                        lgStrSql.AppendLine(" DELETE S_CONFIGURATION ");
                        ...
                        lgStrSql.AppendLine(string.Format(" AND SEQ_NO = {0}", Data.FilterVariable(pDbSaveArgs.Data.Rows[i]["SEQ_NO"])));
            ...
            try {
                switch(cb_args.Cmd) {
                    ...
                    case "DbSave":
                        ...
                        try {
                            lgDbBatAccess.BatchProcessStart("B1002MA1");
