		...
		.apply(
				Ext.form.VTypes,
				{
					'date' : function() {
						 ...
						 * valid dates with 2 digit month, 2 digit day, 4 digit
						 * year. Date separator can be ., -, or /. Uses
						 * combination of regular expressions and string parsing
						 * to validate date. Ex. mm/dd/yyyy or mm-dd-yyyy or
						 * mm.dd.yyyy
						 * 
						 * PARAMETERS: strValue - String to be tested for
						 * validity
						 * 
						 * RETURNS: True if valid, otherwise false.
						 * 
						 * REMARKS: Avoids some of the limitations of the
						 * Date.parse() method such as the date separator
						 * character.
						 ******************************************************/
						...
						return function(strValue) {
							...
							if (!objRegExp.test(strValue))
								return false; // doesn't match pattern, bad
												// date
							else {
								...
								if (arrayLookup[arrayDate[0]] != null) {
									if (intDay <= arrayLookup[arrayDate[0]]
											&& intDay != 0)
										return true; // found in lookup
														// table, good date
								}
								...
								if (intMonth == 2) {
									...
									if (intDay > 0 && intDay < 29) {
										...
										if ((intYear % 4 == 0)
												&& (intYear % 100 != 0)
												|| (intYear % 400 == 0)) {
											// year div by 4 and ((not div by
											// 100) or div by 400) ->ok
											return true;
										}
									}
								}
...
Ext.apply(Ext.form.VTypes, {
	'numeric' : function() {
		 ...
		 * PARAMETERS: strValue - String to be tested for validity RETURNS: True
