...
/// <reference name="System.Xml.js" assembly="System.Xml" />
...
System.Xml.XmlRequest = function(forceXml){
    ...
    if(window.ActiveXObject){
		...
		if (forceXml == true) xmlRequest.overrideMimeType('text/xml');
		return xmlRequest;
    }
}
...
System.Xml.XslTemplate = function(fileName){
    ...
    if(window.ActiveXObject){
		...
		if (fileName){
			xmlRequest.open("GET", fileName, false);
			xmlRequest.send(null);
			var xslTemplate = xmlRequest.responseXML;
		}
        return xslTemplate;
    }
}
    ...
    this.TransformToXmlNode = function(target){
        ...
        if(window.ActiveXObject){
           // FreeThreaded works slower because parser need to manage concurrent access among threads.
    		var xmlResults = new ActiveXObject("Msxml2.DOMDocument");
		    ...
		    // NOTE!!!: It will not produce an HTML DOM object if only the toplevel element of the result is <html xmlns="http://www.w3.org/1999/xhtml">
