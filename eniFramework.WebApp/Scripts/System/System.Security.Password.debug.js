...
/// <reference name="System.Security.Password.js" assembly="System.Security.Password" />
...
System.Security.Password.Interface.AddControls = function(passwordTextBox, confirmTextBox, generateButton, advancedButton, showPassCheckBox, autoIfEmpty){
	...
	if (cp.Advanced){
		System.Security.Password.Interface.Controls.ByAdvanced[cp.id] = cp;
	}
	return cp;
}
...
System.Security.Password.Interface.Attach = function(passwordTextBox, confirmTextBox, generateButton, advancedButton, showPassCheckBox){
	...
	if (cp.Generate){
		var subHandler = function(){ System.Security.Password.Interface.GenerateClickHandler(cp.Password, e); };
		Events.Add(cp.Generate, "click", new System.EventHandler(this, subHandler), true);
	}
	if (cp.Advanced){
		var subHandler = function(){ System.Security.Password.Interface.AdvancedClickHandler(cp.Password, e); };
		Events.Add(cp.Advanced, "click", new System.EventHandler(this, subHandler), true);
	}
}
...
System.Security.Password.Char = function(code){
	//---------------------------------------------------------
	// Public properties.
    this.Char = new String;
    this.Code = new Number;
    this.Case = new String;
...
System.Security.Password.Call = function(name){
    ...
    this.Load = function(callName){
        ...
        for (var property in basicTemplate.Data){
			var item = basicTemplate.Data[property];
			var code = eval(item[0]);
			this.Data[code] =  new Array();
		...
		if (typeof(template) == "object"){
			...
			for (var property in template.Data){
				var item = template.Data[property];
				var code = eval(item[0]);
				this.Data[code] =  new Array();
				this.Data[code][0] = item[1];
				this.Data[code][1] = item[2];
