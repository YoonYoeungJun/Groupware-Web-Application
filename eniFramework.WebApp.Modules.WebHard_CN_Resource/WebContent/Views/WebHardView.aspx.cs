...
namespace eniFramework.WebApp.Modules {
    public class LinqFileSystemProvider : FileSystemProviderBase {
        ...
        public override string RootFolderDisplayName { get { return "大一汽配|有|"/*rootFolderDisplayName*/; } }
    ...
    public partial class WebHardView : BaseUIACTPage {
        ...
        protected override void OnWebUIPageLoad() {
            ...
            strSql.AppendLine(" SELECT C.PLANT_NM_CN, B.DEPT_NM_CN ");
            strSql.AppendLine(" FROM S_USR_MST A ");
            strSql.AppendLine(" LEFT JOIN S_ACCT_DEPT B ON(A.DEPT_CD = B.DEPT_CD) ");
            strSql.AppendLine(" LEFT JOIN S_PLANT C ON(A.PLANT_CD = C.PLANT_CD) ");
            strSql.AppendLine(string.Format(" WHERE A.USR_ID = {0} ", Data.FilterVariable(lgACT.UserId)));

            var dt = lgDbAccess.ExecuteDataSet(strSql.ToString()).Tables[0];

            var dept_path = string.Format("{0} {1}", dt.Rows[0]["PLANT_NM_CN"], dt.Rows[0]["DEPT_NM_CN"]);
            Global.gDirectorys.GetSpecialDirectory(PathKind.WebHard, PathFormatter.FullPhysicalPath, PathType.Default, dept_path);

            Global.gDirectorys.GetSpecialDirectory(PathKind.WebHard, PathFormatter.FullPhysicalPath, PathType.Default, "Cloud\\Fax97700");

            var scan_path = string.Format("Cloud\\{0} {1} 扫描", dt.Rows[0]["PLANT_NM_CN"], dt.Rows[0]["DEPT_NM_CN"]);
            ...
            fmWebhard.SettingsPermissions.AccessRules.Add(new FileManagerFolderAccessRule("Cloud", Rights.Allow) { EditContents = Rights.Allow, Upload = Rights.Deny, Index = 2 });
            ...
            fmWebhard.SettingsPermissions.AccessRules.Add(new FileManagerFolderAccessRule("Cloud\\Fax97700", Rights.Allow) { EditContents = Rights.Allow, Upload = Rights.Deny });
